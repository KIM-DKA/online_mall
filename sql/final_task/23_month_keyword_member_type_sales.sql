-- 23_month_keyword_member_type_sales : 회원 유형별 상품 카테고리 총 구매 금액과 월별 구매 금액을 기록한 테이블입니다.

create table if not exists final_task.month_keyword_member_type_sales as
...
select
    ...
    count(distinct ol.member_code) as unique_users, -- 고유 사용자 수
    (select total_member_count from total_members) as total_member_count -- 전체 회원 수 추가
from
    online_mall.order_log as ol
join
    online_mall.member_info as mi
    on ol.member_code = mi.member_code -- 회원 데이터와 결합하여 회원 유형 정보 추가
where
    ol.product_name similar to '%(항암|미네랄|파이토|유기농)%' -- 키워드 필터
    and ol.purchase_date >= '2024-01-01' -- 2024년 데이터만 포함
group by
    purchase_month, mi.member_type, product_category
order by
    purchase_month, mi.member_type, product_category;
